...
func (s *Server) Handle(writer miekgdns.ResponseWriter, received *miekgdns.Msg) {
	...
	if received.Opcode != miekgdns.OpcodeUpdate {
		Logger.Debugw("query with unsupported opcode", "opcode", received.Opcode)
	...
	if Logger.Level() == zapcore.DebugLevel {
		...
		Logger.Debugw("processing query for a known zone",
			"zone", zoneName, "class", miekgdns.ClassToString[zoneClass],
			"signed", signed, "algorithm", algorithm, "mac_size", macSize, "fudge", fudge,
			"valid", valid, "sig_error", tsigStatus,
			"count_question", len(received.Question), "count_answer", len(received.Answer),
			"count_ns", len(received.Ns), "count_extra", len(received.Extra))
	}
	...
	if tsig != nil && tsigStatus != nil {
		Logger.Debugw("early rejection of an invalid signature")
	...
	if tsig == nil && zone.HasAuthenticationDisabled() == false {
		Logger.Debugw("early rejection of an unauthenticated update to a secured zone")
		response.SetRcode(received, miekgdns.RcodeRefused)
		goto reply
	}

	authorization = update.NewAuthorization(zone)
	...
	if err := update.Execute(authorization, &prerequisites, &received.Ns, Logger); err != nil {
		Logger.Debugw("update execution failed", "error", err.Error())
