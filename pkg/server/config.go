...
type EmbeddedHandlerConfiguration struct {
	...
	Adapter adapters.AdapterSlug `validate:"required,adapterslug"`
}

type HandlerConfiguration struct {
	EmbeddedHandlerConfiguration
	Settings adapters.IAdapterConfiguration
}
...
func validateAdapterSlug(fl validator.FieldLevel) bool {
	if fl.Field().Type() != reflect.TypeFor[adapters.AdapterSlug]() {
		panic("wrong type for adapter slug validation")
	}

	return fl.Field().Interface().(adapters.AdapterSlug).IsValid()
}
